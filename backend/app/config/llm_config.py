...
class AgentsConfig:
    ...
    architect: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=_agent_model("architect", "deepseek-v3.2"),
        temperature=0.7,
        max_tokens=4000
    ))
    ...
    scout: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=_agent_model("scout", "qwen-plus"),
        temperature=0.5,
        max_tokens=4000
    ))
    ...
    data_analyst: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=_agent_model("data_analyst", "deepseek-v3.2"),
        temperature=0.3,
        max_tokens=8000
    ))
    ...
    wizard: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=_agent_model("wizard", "deepseek-v3.2"),
        temperature=0.3,
        max_tokens=8000
    ))
    ...
    critic: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=_agent_model("critic", "deepseek-v3.2"),
        temperature=0.5,
        max_tokens=4000
    ))
    ...
    writer: AgentModelConfig = field(default_factory=lambda: AgentModelConfig(
        model=_agent_model("writer", "deepseek-v3.2"),
        temperature=0.7,
        max_tokens=16000
    ))
...
class LLMConfig:
    ...
    """
    # API 配置
    api_key: str = field(default_factory=lambda: _resolved_model_value("API_KEY", "ollama"))
    base_url: str = field(default_factory=lambda: _resolved_model_value(
    ...
    # 默认模型（用于未单独配置的场景）
    default_model: str = field(default_factory=lambda: _resolved_model_value(
        "MODEL", "deepseek-v3.2"
    ))
