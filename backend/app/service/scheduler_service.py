...
def collection_sources_configured() -> bool:
    """只有至少一个资讯数据源配置完整时才启动定时采集。"""
    return bool(os.getenv("BOCHA_API_KEY") or os.getenv("BID_APP_CODE"))
...
class SchedulerService:
    ...
    def start(self):
        """启动调度器"""
        if not collection_sources_configured():
            logger.info("资讯采集数据源未配置，跳过定时任务")
            return
