...
class ResearchPhase(str, Enum):
    ...
    REVIEW_FAILED = "review_failed"  # 已停止，但未通过质量审核
...
class ResearchState(TypedDict):
    ...
    session_id: str                         # 会话ID
    phase: str                              # 当前阶段
    last_completed_phase: str               # 最近完整结束的原子阶段，恢复游标
    ...
    kb_name: Optional[str]                  # 指定本地知识库；为空时可搜索全部行业库
    ...
    critical_issues: int                    # 当前审核的 critical 数量
    major_issues: int                       # 当前审核的 major 数量
    completion_reason: str                  # 终态原因，不能用 phase 猜测审核结果
