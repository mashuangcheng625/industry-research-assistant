...
- `issues` 只保留最重要的 3 条，`fact_check_results` 最多 5 条。
- `missing_aspects` 和 `strength_points` 各最多 3 条。
- 每个 description、evidence、suggestion、reason 字段尽量不超过 100 字。
...
```"""
    ...
    async def process(self, state: ResearchState) -> ResearchState:
        ...
        review_result = self._normalize_review_result(review_result)
        ...
        if not review_result:
            ...
            state["major_issues"] = 0
            state["unresolved_issues"] = 1
            state["completion_reason"] = "review_output_invalid"
            state["phase"] = ResearchPhase.REVIEW_FAILED.value
            state["errors"].append("审核模型未返回可解析的结构化结果")
            self.add_message(state, "review_failed", {
                "agent": self.name,
                "reason": "review_output_invalid",
                "content": "审核结果不可解析，按失败关闭处理，报告未获批准。",
            })
            return state
        ...
        if review_result:
            ...
            state["critical_issues"] = len(critical_issues)
            state["major_issues"] = len(major_issues)
            state["unresolved_issues"] = len(critical_issues) + len(major_issues)
            ...
            if verdict == "pass":
                ...
                state["completion_reason"] = "review_passed"
                state["phase"] = ResearchPhase.COMPLETED.value
            elif state["iteration"] >= state["max_iterations"]:
                # 迭代耗尽只代表停止自动修订，不代表质量通过。
                state["review_status"] = "rejected"
                state["completion_reason"] = "max_iterations_with_unresolved_issues"
                state["phase"] = ResearchPhase.REVIEW_FAILED.value
                self.add_message(state, "review_failed", {
                    "agent": self.name,
                    "reason": state["completion_reason"],
                    "content": "已达最大审核次数且仍有严重问题，报告未获批准。",
                })
            else:
    ...
    def _normalize_review_result(self, review_result: Dict[str, Any]) -> Dict[str, Any]:
        """用确定性规则消除 LLM 评分、问题严重度和 verdict 的矛盾。"""
        if not review_result:
            return review_result

        issues = review_result.get("issues", [])
        assessment = review_result.setdefault("overall_assessment", {})
        critical_count = sum(issue.get("severity") == "critical" for issue in issues)
        major_count = sum(issue.get("severity") == "major" for issue in issues)
        score = float(assessment.get("quality_score", 0) or 0)

        if critical_count:
            assessment["quality_score"] = min(score, 4.0)
            assessment["verdict"] = "major_issues"
        elif major_count:
            assessment["quality_score"] = min(score, 6.0)
            assessment["verdict"] = "needs_revision"
        elif score < 7 and assessment.get("verdict") == "pass":
            assessment["verdict"] = "needs_revision"
    ...
    async def _review_content(self, state: ResearchState) -> Dict[str, Any]:
        ...
        prompt = self.REVIEW_PROMPT.format(
            ...
            draft_content=draft_content[:6000],  # 为本地小模型保留结构化输出空间
        ...
        response = await self.call_llm(
            system_prompt=(
                "你是一位极其严苛的质量审核专家，专门找出研究报告中的问题。"
                "必须严格遵守数量和字数上限，只输出可解析 JSON。"
            ),
            user_prompt=prompt,
            json_mode=True,
            temperature=0.2,
