...
请为该课题生成与问题直接匹配的三章研究大纲和两个可验证假设。
不要套用“市场概况、竞争格局、政策环境”模板，除非原问题明确要求这些方面。
聚焦技术问题时，优先按“证据与现状—关键缺口—工程建议与待验证项”组织。
搜索词必须面向原始资料中可能存在的技术概念，不得自行加入原问题没有要求的企业、市场数据或政策。

输出 JSON 格式如下：

{{
  "hypothesis_1": "与课题直接相关的可证伪假设",
  "hypothesis_2": "需要用另一类证据验证的假设",
  "sec_1_title": "证据与现状",
  "sec_1_desc": "界定问题并归纳已有证据",
  "sec_1_query": "搜索关键词",
  "sec_1_requires_data": false,
  "sec_1_requires_chart": false,
  "sec_2_title": "关键缺口",
  "sec_2_desc": "按主题分析技术或产业缺口",
  "sec_2_query": "搜索关键词",
  "sec_2_requires_data": false,
  "sec_2_requires_chart": false,
  "sec_3_title": "工程建议与待验证项",
  "sec_3_desc": "区分证据支持的建议和仍需补充的信息",
  "sec_3_query": "搜索关键词",
  "sec_3_requires_data": false,
  "sec_3_requires_chart": false,
  "questions": "核心问题1;核心问题2;核心问题3"
}}

严格只生成 3 个章节。`requires_data` 只有在该章确实需要可核验的数值数据时才设为 true；
`requires_chart` 只有在存在足够结构化数据时才设为 true。"""
    ...
    def _convert_flat_to_outline(self, flat_result: Dict) -> Dict:
        ...
        for i in range(1, 10):  # 最多支持9个章节
            ...
            section = {
                ...
                "section_type": "mixed",
                "requires_data": bool(flat_result.get(f"sec_{i}_requires_data", False)),
                "requires_chart": bool(flat_result.get(f"sec_{i}_requires_chart", False)),
    ...
    async def _initial_planning(self, state: ResearchState) -> ResearchState:
        ...
        for attempt in range(max_retries + 1):
            ...
            if attempt < max_retries:
                ...
                # 简化提示词重试
                prompt = f"""请为"{state['query']}"生成与问题直接匹配的三章研究大纲。

输出JSON格式：
{{"outline": [
    {{"id": "sec_1", "title": "章节标题", "description": "描述", "section_type": "mixed", "requires_data": true, "requires_chart": false, "search_queries": ["关键词1", "关键词2"]}},
    {{"id": "sec_2", "title": "章节标题", "description": "描述", "section_type": "mixed", "requires_data": false, "requires_chart": false, "search_queries": ["关键词"]}},
    {{"id": "sec_3", "title": "章节标题", "description": "描述", "section_type": "mixed", "requires_data": false, "requires_chart": false, "search_queries": ["关键词"]}}
], "research_questions": ["问题1", "问题2", "问题3"], "key_entities": []}}

要求：outline必须且只能包含3个章节，不得套用与原问题无关的通用市场模板。"""
