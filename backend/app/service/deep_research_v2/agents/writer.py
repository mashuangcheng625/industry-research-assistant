...
4. **证据边界**：只能使用“相关事实”明确支持的断言；不得根据“材料未提及”推断某项技术不存在。
5. **引用规范**：只能逐字复制素材中给出的来源 URL，不得生成、补全或猜测任何网址。
6. **图表整合**：只有“相关图表”给出真实 chart ID 时才能引用；“暂无图表”时禁止虚构图表。
7. **字数控制**：本章节 250-400 字
...
3. 撰写结论与展望
4. 整理参考文献列表（确保链接可点击）
5. 严格保留已给出的来源 URL，不得生成新 URL、新数字、新日期或新图表。
6. 只输出原始 JSON；`full_report` 控制在 1200-1800 个中文字符，
   `conclusions` 最多 3 条，`references` 最多 5 条。
...
```json
{{
    "executive_summary": "执行摘要（150-250字）",
...
## 任务
根据反馈修订报告，解决指出的问题。
只输出原始 JSON，修订后正文尽量不超过 1800 个中文字符。
...
```"""
    ...
    def _sanitize_markdown_evidence(
        self,
        content: str,
        allowed_urls: set[str],
        allowed_chart_ids: set[str],
    ) -> str:
        """删除 Writer 在证据白名单之外生成的链接和图表引用。"""
        if not content:
            return content

        def replace_image(match: re.Match) -> str:
            alt, target = match.group(1), match.group(2).strip()
            if target in allowed_chart_ids:
                return match.group(0)
            return f"[{alt}：无可验证图表]" if alt else ""

        sanitized = re.sub(r"!\[([^\]]*)\]\(([^)]+)\)", replace_image, content)
        ...
        return re.sub(r"(?<!!)\[([^\]]+)\]\(([^)]+)\)", replace_link, sanitized)
    ...
    async def _write_section(self, state: ResearchState, section: Dict) -> None:
        ...
        if result and result.get("content"):
            ...
            allowed_chart_ids = {
                ...
                if chart.get("id") and chart.get("section_id") == section_id
            }
            section_content = self._sanitize_markdown_evidence(
                result["content"],
                allowed_urls,
                allowed_chart_ids,
            )

            # 小模型可能忘记复制 URL；用检索事实确定性补齐证据条目。
            evidence_sources = []
            seen_urls = set()
            for fact in related_facts:
    ...
    async def _synthesize_report(self, state: ResearchState) -> None:
        ...
        if result and result.get("full_report"):
            state["final_report"] = self._sanitize_markdown_evidence(
                result.get("full_report", ""),
                self._allowed_source_urls(state),
                {chart.get("id") for chart in state.get("charts", []) if chart.get("id")},
            )
