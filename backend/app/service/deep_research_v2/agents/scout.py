...
try:
    ...
    try:
        ...
        from app.config.industry_config import INDUSTRY_CONFIGS
        MILVUS_AVAILABLE = True
    except ImportError:
...
- `extracted_facts` 最多 3 条，每条只保留最关键的 1 个 `data_points`。
- `hypothesis_evidence` 最多 3 条，`entities_discovered` 最多 3 条。
- `key_insights`、`follow_up_queries`、`source_tracing_queries` 各最多 2 条。
- `missing_info` 最多 3 条，每个文本字段尽量控制在 100 字内。
...
```"""
    ...
    def __init__(
        ...
        # 本地检索服务按查询懒加载，避免只使用网络模式时也强制
        # 建立 Milvus 连接。
        self.local_search_available = MILVUS_AVAILABLE
    ...
    async def _supplementary_research(self, state: ResearchState) -> ResearchState:
        ...
        for query in pending_queries[:5]:  # 最多处理5个补充查询
            ...
            # 继承用户选择的搜索模式，不在补充研究中偷偷切换到网络。
            results = await self._execute_configured_search(state, query, count=8)
            ...
            if results:
                ...
                if analysis:
                    analysis["extracted_facts"] = [
                        self._ground_fact_source(fact, results)
    ...
    async def _fetch_stock_data_if_relevant(self, state: ResearchState) -> None:
        ...
        当用户查询涉及半导体上市公司时（如“中芯国际近期市场信号”），
        自动获取行情并作为带时点的市场证据，不将其等同于基本面结论。
        """
        try:
            try:
...
```"""
    ...
    async def _research_section(self, state: ResearchState, section: Dict) -> None:
        ...
        if analysis:
            analysis["extracted_facts"] = [
                self._ground_fact_source(fact, all_results)
                for fact in analysis.get("extracted_facts", [])
            ]
            ...
            local_only = state.get("search_local", False) and not state.get("search_web", True)
            if (
                source_tracing
                and not local_only
    ...
    def _ground_fact_source(
        self,
        fact: Dict[str, Any],
        search_results: List[Dict[str, Any]],
    ) -> Dict[str, Any]:
        """将 LLM 输出的事实限定到本次检索实际返回的信源。"""
        grounded = dict(fact)
        result_by_url = {
        ...
        claimed_url = grounded.get("source_url", "")
        matched = result_by_url.get(claimed_url)

        # 只有一条证据时，即使小模型没有准确复制 URL，也可以唯一定位。
        if matched is None and len(search_results) == 1:
            matched = search_results[0]
        elif matched is None and len(result_by_url) == 1:
            # 多个向量切片可能同属一份文档，此时仍可唯一定位信源。
            matched = next(iter(result_by_url.values()))

        if matched is not None:
            grounded["source_url"] = matched.get("url", "")
            grounded["source_name"] = (
                matched.get("site_name")
                or matched.get("title")
                or grounded.get("source_name", "")
            )
            grounded["_source_metadata"] = {
                "evidence_origin": (
                    "local_knowledge_base" if matched.get("is_local") else "web_search"
                ),
        ...
        grounded["source_url"] = ""
        grounded["needs_verification"] = True
        grounded["credibility_score"] = min(
            float(grounded.get("credibility_score", 0.3)),
            0.3,
        )
        grounded["_source_metadata"] = {"evidence_origin": "unresolved"}
        return grounded
    ...
    async def _execute_deep_search(
        ...
        # 对固定的本地语料库，深层递归往往只会反复命中同一文档。
        # 保留一次查询改写能力，同时避免小模型调用数指数增长。
        if state.get("search_local", False) and not state.get("search_web", True):
        ...
        for query in queries:
            # 递归搜索必须继承顶层的 web/local 选择。
            results = await self._execute_configured_search(state, query, count=6)
    ...
    async def _execute_configured_search(
        self,
        state: ResearchState,
        query: str,
        count: int = 8,
        ...
        if state.get("search_local", False):
            results.extend(await self._execute_local_search(
                query,
                top_k=count,
                kb_name=state.get("kb_name"),
            ))
...
3. 只输出原始 JSON；`extracted_facts` 最多 3 条，`data_points` 最多 2 条，
   `further_tracing_queries` 最多 1 条，每个文本字段尽量不超过 100 字。
    ...
    async def _execute_local_search(
        ...
        try:
            kb_names = [kb_name] if kb_name else [
                config.knowledge_base_name for config in INDUSTRY_CONFIGS.values()
            ]
            self.logger.info(
                "Executing local knowledge base search: %s, kb=%s",
                query[:50],
                kb_names,
            )
            results = []
            per_kb_top_k = top_k if kb_name else max(3, top_k // len(kb_names))
            for selected_kb in kb_names:
                ...
                for result in kb_results:
                    results.append({**result, "selected_kb": selected_kb})
            ...
            for r in results:
                formatted_results.append({
                    'url': f"local://kb/{r.get('selected_kb', 'unknown')}/{r.get('document_id', 'unknown')}",
                    'title': r.get('document_name', 'N/A'),
                    'summary': r.get('content_with_weight', '')[:1200],
                    'snippet': r.get('content_with_weight', '')[:300],
                    'site_name': f"本地知识库/{r.get('selected_kb', 'unknown')}",
                    'date': '',
                    'score': r.get('score', 0),
                    'is_local': True,
...
""")
        ...
        response = await self.call_llm(
            system_prompt=(
                "你是专业的研究分析师，擅长从搜索结果中提取"
                "结构化信息、验证假设并评估来源质量。"
                "必须严格遵守数量上限，只输出可解析的 JSON。"
            ),
            user_prompt=prompt,
            json_mode=True,
            temperature=0.2
