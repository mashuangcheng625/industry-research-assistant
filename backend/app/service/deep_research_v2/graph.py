...
# 取消控制必须位于 core，不能从 router 反向导入，否则 router -> service ->
# graph -> router 会形成循环依赖并悄悄落入“永不取消”的 fallback。
try:
    from core.research_control import is_research_cancelled, clear_cancel_flag
except ImportError:
    from app.core.research_control import is_research_cancelled, clear_cancel_flag
...
class DeepResearchGraph:
    ...
    def __init__(
        ...
        self.agent_timeout_seconds = max(
            1.0,
            float(os.getenv("AGENT_TIMEOUT_SECONDS", "180")),
        )
    ...
    def _save_checkpoint(
        ...
        try:
            ...
            if checkpoint_id:
                ...
                CHECKPOINT_OPERATIONS.labels(operation="save", outcome="success").inc()
                return True
        except Exception as e:
            logger.warning(f"Failed to save checkpoint: {e}")

        CHECKPOINT_OPERATIONS.labels(operation="save", outcome="failure").inc()
        return False
    ...
    def _load_checkpoint(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if state:
                ...
                CHECKPOINT_OPERATIONS.labels(operation="load", outcome="hit").inc()
                return state
            CHECKPOINT_OPERATIONS.labels(operation="load", outcome="miss").inc()
        except Exception as e:
            logger.warning(f"Failed to load checkpoint: {e}")
            CHECKPOINT_OPERATIONS.labels(operation="load", outcome="failure").inc()
    ...
    async def run(
        ...
        Yields:
            SSE 事件字典
        """
        # 清除的是上一次运行遗留的取消标志。必须发生在任何 start/resumed
        # 事件之前，不能覆盖用户看到 start 后发出的新取消请求。
        if session_id:
        ...
        if resume and session_id:
            ...
            if state:
                # 只用于本次调度，不写入持久化状态。phase 表示当前阶段，
                # last_completed_phase 才是安全恢复游标。
                state["_resume_from_phase"] = state.get("phase", ResearchPhase.INIT.value)
                if "last_completed_phase" in state:
                    state["_resume_after_phase"] = state.get("last_completed_phase", "")
                else:
                    # 兼容旧 checkpoint 的“phase 已完成”语义。
                    state["_resume_after_phase"] = state.get("phase", "")
                    state["last_completed_phase"] = state["_resume_after_phase"]
                state.setdefault("review_status", "pending")
                state.setdefault("critical_issues", 0)
                state.setdefault("major_issues", 0)
    ...
    async def _run_simplified(self, state: ResearchState) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        async def run_agent_with_streaming(agent):
            ...
            state["_active_agent_task"] = task
            started_at = asyncio.get_running_loop().time()

            def observe_agent_task(done: asyncio.Task) -> None:
                if done.cancelled():
                    outcome = "cancelled"
                else:
                    try:
                        outcome = "error" if done.exception() is not None else "success"
                    except asyncio.CancelledError:
                        outcome = "cancelled"
                duration = max(0.0, time.perf_counter() - agent_started_at)
                AGENT_RUNS.labels(agent=agent.name, outcome=outcome).inc()
                AGENT_DURATION.labels(agent=agent.name, outcome=outcome).observe(duration)

            agent_started_at = time.perf_counter()
            task.add_done_callback(observe_agent_task)
            timeout_seconds = max(
                0.01,
                float(getattr(self, "agent_timeout_seconds", 180.0)),
            )
            ...
            while not task.done():
                ...
                if remaining <= 0:
                    AGENT_TIMEOUTS.labels(agent=agent.name).inc()
                    task.cancel()
                    try:
                        await task
                    except asyncio.CancelledError:
                        pass
                    message = f"Agent {agent.name} exceeded {timeout_seconds:.2f}s timeout"
            ...
            try:
                ...
                state["errors"].append(f"Agent {agent.name} failed: {e}")
                raise
            finally:
                if task.done():
                    state["_active_agent_task"] = None
        ...
        resume_phase = state.get("_resume_from_phase")
        resume_after_phase = state.get("_resume_after_phase", "")
        resume_order = {
            "": 0,
            ResearchPhase.INIT.value: 0,
            ResearchPhase.PLANNING.value: 1,
            ResearchPhase.RESEARCHING.value: 2,
            ResearchPhase.ANALYZING.value: 3,
            ResearchPhase.WRITING.value: 4,
            ResearchPhase.REVIEWING.value: 4,
            ResearchPhase.COMPLETED.value: 5,
            ResearchPhase.REVIEW_FAILED.value: 5,
        }
        if resume_after_phase not in resume_order:
            raise ValueError(f"不支持从未知完成阶段恢复: {resume_after_phase}")
        resume_index = resume_order[resume_after_phase]
        ...
        try:
            ...
            if resume_index <= 0:
                ...
                async for msg in run_agent_with_streaming(self.architect):
                    yield msg
                state["messages"] = []
                state["last_completed_phase"] = ResearchPhase.PLANNING.value
            else:
                yield {"type": "phase_skipped", "phase": "planning", "reason": "checkpoint_completed"}
            # 保存检查点（含步骤信息）
            ...
            if resume_index <= 1:
                ...
                async for msg in run_agent_with_streaming(self.scout):
                    yield msg
                state["messages"] = []
                state["last_completed_phase"] = ResearchPhase.RESEARCHING.value
            else:
                yield {"type": "phase_skipped", "phase": "researching", "reason": "checkpoint_completed"}
            # 保存检查点（含步骤信息）
            cp_event = await save_checkpoint_async({
                "type": "researching",
                "status": "completed",
            ...
            if resume_index <= 2:
                ...
                requires_analysis = any(
                    section.get("requires_data", False) or section.get("requires_chart", False)
                ...
                if requires_analysis:
                    ...
                    yield {
                        ...
                        "content": "当前任务无可核验的结构化数据需求，跳过图表与代码分析。",
                    }
                state["last_completed_phase"] = ResearchPhase.ANALYZING.value
            else:
                yield {"type": "phase_skipped", "phase": "analyzing", "reason": "checkpoint_completed"}
            # 保存检查点（含步骤信息）
            cp_event = await save_checkpoint_async({
                "type": "analyzing",
                "status": "completed",
            ...
            if resume_index <= 3:
                ...
                async for msg in run_agent_with_streaming(self.writer):
                    yield msg
                state["messages"] = []
                state["last_completed_phase"] = ResearchPhase.WRITING.value
            else:
                yield {"type": "phase_skipped", "phase": "writing", "reason": "checkpoint_completed"}
            # 保存检查点（含步骤信息）
            cp_event = await save_checkpoint_async({
                "type": "writing",
                "status": "completed",
            ...
            # 恢复点可能恰好位于审核已经作出路由决定之后。
            if resume_phase == ResearchPhase.REVISING.value:
                yield {"type": "phase", "phase": "revising", "content": "从检查点继续修订报告..."}
                state["phase"] = ResearchPhase.REVISING.value
                async for msg in run_agent_with_streaming(self.writer):
                    yield msg
                state["messages"] = []
                state["last_completed_phase"] = ResearchPhase.WRITING.value
            elif resume_phase == ResearchPhase.RE_RESEARCHING.value:
                yield {"type": "phase", "phase": "re_researching", "content": "从检查点继续补充搜索..."}
            ...
            while state["iteration"] < state["max_iterations"]:
                ...
                async for msg in run_agent_with_streaming(self.critic):
                    yield msg
                state["messages"] = []
                state["last_completed_phase"] = ResearchPhase.REVIEWING.value

                # 评审会把流程路由到完成、修订或补充检索；立即保存该决定，
                # 避免恢复任务时退回到评审前的草稿状态。
                cp_event = await save_checkpoint_async({
                    "type": "reviewing",
                    "status": "completed",
                ...
                if state["phase"] == ResearchPhase.COMPLETED.value:
                    break
                if state["phase"] == ResearchPhase.REVIEW_FAILED.value:
                    break
            ...
            if not review_approved:
                state["phase"] = ResearchPhase.REVIEW_FAILED.value
                state["review_status"] = "rejected"
                if not state.get("completion_reason"):
                    state["completion_reason"] = "review_not_approved"
                if self.checkpoint_service and session_id:
                    update_ui_state()
                    self._save_checkpoint(state, user_id, ui_state)
                ...
                yield {
                    ...
                    "message": "研究草稿未通过质量审核，不应作为最终结论使用。",
                }
                return
            ...
            yield {
                ...
                "completion_reason": state.get("completion_reason", "review_passed"),
            ...
            active_task = state.get("_active_agent_task")
            ...
            if (
                session_id
                and is_research_cancelled(session_id)
                and self.checkpoint_service
            ):
                state["completion_reason"] = "cancelled_by_user"
                update_ui_state()
                self._save_checkpoint(state, user_id, ui_state)
                self.checkpoint_service.update_status(
                    session_id,
