...
RUN_LOCK_KEY_PREFIX = "research:run-lock:"
RUN_LOCK_TTL_SECONDS = max(
    60,
    int(os.getenv("RESEARCH_RUN_LOCK_TTL_SECONDS", "3600")),
)
...
def get_research_service_v2(max_iterations: Optional[int] = None):
    """获取 V2 研究服务实例（使用配置文件中的模型设置）"""
    # 直接创建服务，配置从 llm_config.py 读取
    return DeepResearchV2Service(max_iterations=max_iterations)


def acquire_research_run_lock(session_id: str) -> str:
    """为一个研究 session 获取跨进程互斥锁。"""
    owner_token = str(uuid.uuid4())
    acquired = cache.acquire_lock(
        f"{RUN_LOCK_KEY_PREFIX}{session_id}",
        owner_token,
        expire=RUN_LOCK_TTL_SECONDS,
    )
    if acquired is None:
        RUN_LOCK_OPERATIONS.labels(operation="acquire", outcome="unavailable").inc()
        raise HTTPException(
            status_code=HTTP_503_SERVICE_UNAVAILABLE,
            detail="Redis unavailable; cannot guarantee exclusive research execution",
        )
    if not acquired:
        RUN_LOCK_OPERATIONS.labels(operation="acquire", outcome="contention").inc()
        raise HTTPException(
            status_code=HTTP_409_CONFLICT,
            detail="Research session is already running",
        )
    RUN_LOCK_OPERATIONS.labels(operation="acquire", outcome="success").inc()
    return owner_token


def release_research_run_lock(session_id: str, owner_token: str) -> None:
    """按 owner token 释放 session 锁。"""
    released = cache.release_lock(
        f"{RUN_LOCK_KEY_PREFIX}{session_id}",
        owner_token,
    )
    if released is None:
        RUN_LOCK_OPERATIONS.labels(operation="release", outcome="unavailable").inc()
        logger.error("Failed to release research run lock for session %s", session_id)
    elif released:
...
async def stream_research(
    ...
    if request.version == "v2":
        ...
        lock_token = acquire_research_run_lock(session_id)
        ...
        async def generate_sse_v2():
            try:
                ...
                release_research_run_lock(session_id, lock_token)
...
async def cancel_research(session_id: str):
    ...
    try:
        # Redis 故障时不能谎报取消成功。
        if not request_research_cancel(session_id, expire=300):
            CANCEL_REQUESTS.labels(outcome="unavailable").inc()
            raise HTTPException(
                status_code=HTTP_503_SERVICE_UNAVAILABLE,
                detail="Redis unavailable; cancellation request was not persisted",
            )
        logger.info(f"Research cancelled for session: {session_id}")
        CANCEL_REQUESTS.labels(outcome="accepted").inc()
        return {"success": True, "message": "Research cancellation requested"}
    except HTTPException:
        raise
    except Exception as e:
        CANCEL_REQUESTS.labels(outcome="error").inc()
