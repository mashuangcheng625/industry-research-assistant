...
class ChatRequest(BaseModel):
    ...
    question: str = Field(
        ...,
        min_length=1,
        max_length=CHAT_QUESTION_MAX_CHARS,
        description=f"用户问题，最多 {CHAT_QUESTION_MAX_CHARS} 个字符",
    )
    kb_name: Optional[str] = Field(None, description="要检索的本地知识库名称")
    search_knowledge: bool = Field(False, description="是否搜索知识库")
    search_web: bool = Field(False, description="是否搜索网络")
    model_mode: Optional[Literal["local", "cloud", "auto"]] = Field(
        None,
        description="生成模型路由：本地、云端或自动",
    )
