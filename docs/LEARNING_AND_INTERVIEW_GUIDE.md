...
这不是“把 PDF 放进向量库再调一次大模型”，而是一个面向半导体产业研究的
证据系统：它治理资料的来源和授权，将长文档变成可定位证据，用混合检索
定位证据，约束模型只基于证据回答，将论断绑定到引用，并对资料不足、
运行中断、审核失败和并发退化给出可观测结果。
...
| 100 tests | 本地工程门禁 100/100 通过 | “系统没有 bug” |
