...
| schema | Compose 以 Alembic 为单一来源，CI 覆盖往返迁移和备份恢复 | 防止 ORM drift，并证明备份可恢复 | 生产前补定时备份、异地保存和恢复时间指标 |
...
答案的情况下做网格/贝叶斯搜索、分领域稳定性分析，最后用经过来源预校验的新 blind-v3 验收。
...
至少包括：经过来源预校验的独立 blind-v3 与专家标注、大规模 sparse index、更强 grounding verifier、
定时异地备份、TLS/密钥托管、多副本、分布式 trace 后端和持续 soak test。
...
| 457 tests | 工程门禁包含 453 unit + Milvus Lite / Redis / Alembic / PostgreSQL 备份恢复 4 integration | “系统没有 bug” |
