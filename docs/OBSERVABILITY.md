...
| 任务 | `industry_background_task_queue_depth` | 无 | Redis Stream 待投递加 pending 的总积压 |
| 任务 | `industry_background_tasks_total` | `task_type`, `outcome` | 文档、附件与研究任务的尝试结果 |
| 任务 | `industry_background_task_duration_seconds` | `task_type`, `outcome` | 独立 Worker 的任务尝试耗时 |
...
`host.docker.internal:8000` 抓取宿主机后端，并从 `task-worker:8001` 抓取独立 Worker。配置内含 Agent 超时、研究运行错误、checkpoint 失败和
Redis 锁不可用、持久任务 Worker 掉线、队列积压和任务失败等七条告警规则。
...
端到端与阶段 P95、LLM 延迟/token、checkpoint/锁故障、Critic 审核结果和持久任务积压/结果。
