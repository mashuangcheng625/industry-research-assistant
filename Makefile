...
MODEL_MODE ?= local
MAX_LATENCY ?= 20
ANSWER_REPORT ?= reports/semiconductor_rag_answers_regression_claim_citation_latest.json
...
.PHONY: setup-backend test-backend check-backend-deps check-backend-import lint-frontend build-frontend build-images validate-compose validate-observability validate-sources smoke-ingest-lite audit-ingestion ablate-retrieval-development evaluate-answers-regression demo-rag load-test-chat stress-context-budget run-backend-structured evaluate-answers-regression-structured run-backend-semantic evaluate-answers-regression-semantic build-eval-public validate-eval validate-eval-private check
...
test-backend:
	cd backend && PYTHONPATH=app ../$(PYTHON) -m pytest -q -W error::DeprecationWarning
...
check-backend-import:
	cd backend && PYTHONPATH=app ../$(PYTHON) -W error::DeprecationWarning -c \
...
check: check-backend-deps test-backend check-backend-import lint-frontend build-frontend validate-compose validate-sources validate-eval
