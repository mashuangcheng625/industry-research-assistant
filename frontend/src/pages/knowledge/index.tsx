...
const STATUS_MAP: Record<string, { color: string; text: string }> = {
  ...
  processing: { color: 'processing', text: '正在解析' },
  completed: { color: 'success', text: '可检索' },
...
export default function KnowledgePage() {
  ...
  const handleViewChunks = (doc: KBDocument) => {
    if (doc.status !== 'completed') {
      message.warning('文档解析完成后才能查看文本切片')
      return
    }
            ...
            {(knowledgeBases as KnowledgeBase[]).map((kb) => (
              ...
              >
                ...
                <div className={styles['card-actions']}>
                  ...
                  <Popconfirm
                    title="确定删除此知识库？"
                    description="删除后，知识库及其中的文档将无法恢复。"
                    onConfirm={(e) => {
                      e?.stopPropagation()
    ...
    <div className={styles['knowledge-page']}>
      ...
      >
        ...
        >
          ...
          <Form.Item name="description" label="资料范围与用途">
            <Input.TextArea
              placeholder="说明知识库包含哪些资料、适合回答什么问题（可选）"
              rows={3}
              autoComplete="off"
            />
          </Form.Item>
