  ...
  return (
    <>
      {source.knowledge?.length ? (
        <Section title="知识库来源" icon={IconSource} defaultOpen>
      ...
      {source.web?.length ? (
        <Section title="公开信息来源" icon={IconSource} defaultOpen>
          <div className={styles['chat-message-result__source']}>
...
const 图像 = (props: { item: API.ChatItem }) => {
  ...
  return (
    <Section title="相关图像" icon={IconImage} defaultOpen>
          ...
          >
            <div className={styles['react-step-header']}>
              ...
              {step.tool === 'parallel_search' && (
                <span className={styles['react-step-tool']}>并行检索</span>
              )}
              {isClickable && (
    ...
    () => [
      {
        ...
        renderer(token) {
          ...
          return `<a class="refrence-token" href="${target?.link || 'javascript: void 0'}" target="${target?.link ? '_blank' : '_self'}">[${id}]</a>`
        },
      },
    ],
    [item.reference],
