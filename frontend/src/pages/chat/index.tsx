  ...
  const handleUploadAttachment = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        message.error(`研究资料上传失败：${e.message || '请稍后重试'}`)
        // 移除失败的附件
      ...
      try {
        ...
        if (target.type === ChatType.Deepsearch) {
          res = await api.session.deepsearch({
            ...
            kb_name: deviceState.knowledgeBaseName || undefined,
          ...
          res = await api.session.chatWithAttachments({
            ...
            kb_name: deviceState.knowledgeBaseName || undefined,
            search_knowledge: (deviceState.searchModes as string[]).includes(
              'local',
            ),
            search_web: false,
          })
      ...
      function parseData(slice: string) {
        try {
          ...
          if (target.type === ChatType.Deepsearch) {
            ...
            // 审核未通过是终止态，但不是完成态。草稿可以查看，不能冒充最终报告。
            if (json.type === 'research_review_failed') {
              const warning =
                '⚠️ 研究草稿未通过质量审核，不应作为最终结论使用。'
              target.content = json.final_report
                ? `> ${warning}\n\n${json.final_report}`
                : warning
              target.loading = false
              ...
              target.reactSteps.push({
                step: target.reactSteps.length + 1,
                type: 'observation',
                content: `${warning}\n原因：${json.reason || 'review_not_approved'}；critical=${json.critical_issues || 0}，major=${json.major_issues || 0}`,
                timestamp: Date.now(),
              })
              if (json.references && json.references.length > 0) {
                target.reference = json.references.map(
                  (ref: any, i: number) => ({
                    id: i + 1,
                    title: ref.title || ref.source || ref.source_name || '来源',
              ...
              const writingType = researchDetailsRef.current.has('writing')
                ...
                : 'generating'
              const detail = researchDetailsRef.current.get(writingType)
              if (detail && json.final_report) {
                detail.streamingReport = `> ${warning}\n\n${json.final_report}`
                setSelectedResearchDetail({ ...detail })
              }
              setResearchSteps((prev) =>
            ...
            if (json.type === 'thought') {
              ...
              let displayContent = ''
              if (isParallel) {
                displayContent = `并行检索${section ? `（${section}）` : ''} ${queries.length} 个问题：\n${queries.map((q: string) => `• ${q}`).join('\n')}`
              } else {
                displayContent = `执行研究任务：${tool}${section ? ` · ${section}` : ''}`
              }
              ...
              if (sectionContent) {
                ...
                target.reactSteps.push({
                  step: target.reactSteps.length + 1,
                  type: 'observation',
                  content: `✍️ 章节「${sectionTitle}」已写入研究报告\n字数：${sectionContent.length}\n要点：${(content.key_points || []).slice(0, 2).join('、') || '暂未提取'}`,
                  timestamp: Date.now(),
                })
              }
              ...
              target.reactSteps.push({
                step: target.reactSteps.length + 1,
                type: 'thought',
                content: `🔍 质量检查结果：${score}/10\n${passed ? '✅ 质量检查通过' : '⚠️ 研究结果需要进一步修订'}`,
                timestamp: Date.now(),
              })
  ...
  const send = useCallback(
    async (message: string, attachmentIds?: string[]) => {
      ...
      chat.list.push({
        ...
        type: (deviceState.searchModes as string[]).includes('web')
          ? ChatType.Deepsearch
          : ChatType.Normal,
        content: '',
      })
